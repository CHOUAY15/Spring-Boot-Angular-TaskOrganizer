...
export class MembersComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.route.params.subscribe(params => {
      ...
      this.teamName = params['teamName']; // Renamed 'nomEquipe' to 'teamName' for consistency
  ...
  loadMembers() {
    const loadedData$ = this.loadingService.loadData(
      this.memberService.findByTeam(this.teamId).pipe(tap(members => console.log('Members received', members))),
      400
    );
